...
import { MINIBOYS_QUERY, MODEL_CATEGORIES_QUERY } from "@/sanity/lib/queries";
...
export async function generateMetadata({ params }) {
  ...
  const { data: category } = await sanityFetch({
    ...
    params: { category: "mini-boys", locale },
  });

  return {
    title: category?.seoTitle ?? "Mini-Boys | MiniModels",
...
export default async function MiniBoysPage({ params }) {
  ...
  const nameColor = NameColorSwitcher("mini-boys");

  const { data: models } = await sanityFetch({
    query: MINIBOYS_QUERY,
    ...
    <main className="mx-auto min-h-screen bg-white pt-14 lg:pt-20 pb-12">
      ...
      <section className="mx-auto container max-w-7xl pt-12">
        {models?.length === 0 ? (
          ...
          <div className="flex flex-row flex-wrap justify-center gap-2 transition-all duration-200">
